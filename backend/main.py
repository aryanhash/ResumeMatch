        ...
        try:
            ...
            if len(processing_results) > 1000:  # Safety limit
                ...
                for key in keys_to_remove:
                    del processing_results[key]
                logger.warning(f"🧹 Cleaned up {len(keys_to_remove)} results (memory limit)")
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("🚀 AutoApply AI Backend Starting...")
    ...
    logger.info(f"📋 Multi-agent pipeline ready")
    logger.info(f"🔑 Together API Key: ✓ Set")
    logger.info(f"🌐 CORS Origins: {ALLOWED_ORIGINS}")
    
    # Start cleanup task
    cleanup_task = asyncio.create_task(cleanup_old_results())
    logger.info("🧹 Started result cleanup task")
    ...
    try:
        ...
        pass
    # Shutdown
    logger.info("👋 AutoApply AI Backend Shutting Down...")
...
def run_pipeline(resume_text: str, jd_text: str, progress_callback: Optional[Callable] = None) -> AutoApplyResult:
    ...
    start_time = time.time()
    logger.info(f"🚀 Starting pipeline - Resume: {len(resume_text)} chars, JD: {len(jd_text)} chars")
    ...
    logger.info(f"   ✓ Found {len(parsed_resume.skills)} skills, {len(parsed_resume.experience)} experiences")
    ...
    logger.info(f"   ✓ Match: {gap_analysis.overall_match_percentage}%")
    ...
    logger.info("✨ Step 4: Enhancing skills...")
    ...
    logger.info("🎯 Step 5: OUMI ATS classification...")
    ...
    logger.info(f"   ✓ ATS Bucket: {oumi_result.get('ats_bucket', 'N/A')}")
    ...
    logger.info("📊 Step 6: ATS scoring...")
    ...
    logger.info(f"   ✓ ATS Score: {ats_score.overall_score}/100 ({ats_score.bucket.value})")
    logger.info(f"   📊 Components: skill={ats_score.skill_match_score}, keyword={ats_score.keyword_score}, format={ats_score.formatting_score}, exp={ats_score.experience_alignment_score}")
    ...
    logger.info("✍️ Step 7: Rewriting resume...")
    ...
    logger.info("📚 Step 10: Generating project recommendations...")
    ...
    logger.info(f"   ✓ {len(project_recommendations.recommended_projects)} projects recommended")
    ...
    logger.info(f"✅ Pipeline complete! Total time: {processing_time:.2f}s")
...
def process_async(workflow_id: str, resume_text: str, jd_text: str):
    ...
    logger.info(f"🔄 Async processing started: {workflow_id}")
    try:
        ...
        logger.info(f"✅ Async processing completed: {workflow_id}")
    except Exception as e:
        logger.error(f"❌ Async processing failed: {workflow_id} - {str(e)}")
        processing_results[workflow_id] = {
            "status": "failed",
            "error": str(e),
...
async def process_resume(
    ...
    """
    logger.info(f"📥 Process request received - Resume: {len(text_request.resume_text)} chars, JD: {len(text_request.job_description)} chars")
    ...
    try:
        ...
        logger.error(f"❌ Process request failed: {str(e)}", exc_info=True)
        return ProcessResponse(
            success=False,
            error=str(e)
        )
...
async def process_resume_stream(
    ...
    logger.info(f"📥 Stream process request received - Workflow ID: {workflow_id}")
...
async def parse_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        logger.info(f"📄 Parsed file: {file.filename}, extracted {len(text)} chars")
...
async def download_optimized_resume_pdf(
    ...
    try:
        ...
        pdf_bytes = generate_resume_pdf(
            enhanced_resume,  # ✅ Use enhanced version
...
async def download_optimized_resume_word(
    ...
    try:
        ...
        logger.error(f"❌ Word document generation error: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
